...
function hideCanvas(){host.classList.add('hidden');canvasTab?.classList.remove('active');}
...
function addResizeHandles(element:HTMLElement,node:CanvasNode){
  ...
  for(const direction of ['n','ne','e','se','s','sw','w','nw'] as ResizeDirection[]){
    ...
    h.addEventListener('pointerdown',event=>{if(event.button!==0)return;event.preventDefault();event.stopPropagation();h.setPointerCapture(event.pointerId);selectNode(node.id);element.classList.add('resizing');const sx=event.clientX,sy=event.clientY,start={x:node.x,y:node.y,width:node.width,height:node.height};
...
function renderNode(node:CanvasNode):HTMLElement{
  ...
  if(node.type==='text'){
    ...
    textarea.addEventListener('pointerdown',event=>{event.stopPropagation();selectNode(node.id);});
  ...
  dragHandle.addEventListener('pointerdown',event=>{
    ...
    event.preventDefault();event.stopPropagation();dragHandle.setPointerCapture(event.pointerId);selectNode(node.id);drag={startX:event.clientX,startY:event.clientY,nodeX:node.x,nodeY:node.y};element.classList.add('dragging');
  });
...
viewport.addEventListener('pointerdown',event=>{if((event.target as HTMLElement).closest('.canvas-node')||event.button!==0)return;selectNode(null);viewport.setPointerCapture(event.pointerId);pan={x:event.clientX,y:event.clientY,panX,panY};viewport.classList.add('panning');});
