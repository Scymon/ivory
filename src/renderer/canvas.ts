...
host.innerHTML = `
  <div class="canvas-toolbar">
    <button data-canvas-action="add-text" title="Add text card">＋ Text</button>
    <button data-canvas-action="zoom-out" title="Zoom out">−</button>
    <span class="canvas-zoom">100%</span>
    <button data-canvas-action="zoom-in" title="Zoom in">＋</button>
    <button data-canvas-action="fit" title="Fit content">Fit</button>
  </div>
...
function ensureCanvasTab(path: string): void {
  ...
  close.addEventListener('click', (event) => { event.stopPropagation(); hideCanvas(); canvasTab?.remove(); canvasTab = null; canvasPath = null; });
...
function renderEdges(): void {
  ...
  for (const edge of documentState.edges ?? []) {
    ...
    if (edge.color) path.dataset.color = edge.color;
    edgeSvg.append(path);
  }
}
...
function renderNode(node: CanvasNode): HTMLElement {
  ...
  header.append(title); element.append(header);

  const body = document.createElement('div'); body.className = 'canvas-node-body';
  if (node.type === 'text') {
    const textarea = document.createElement('textarea'); textarea.value = node.text ?? ''; textarea.spellcheck = true;
    textarea.addEventListener('input', () => { node.text = textarea.value; scheduleSave(); });
    body.append(textarea);
  } else if (node.type === 'file') {
    const button = document.createElement('button'); button.className = 'canvas-file-card'; button.textContent = node.file;
    body.append(button);
...
function fitCanvas(): void {
  ...
  const minX = Math.min(...documentState.nodes.map((n) => n.x));
  const minY = Math.min(...documentState.nodes.map((n) => n.y));
  const maxX = Math.max(...documentState.nodes.map((n) => n.x + n.width));
  const maxY = Math.max(...documentState.nodes.map((n) => n.y + n.height));
  ...
  panX = rect.width / 2 - (minX + contentW / 2) * zoom;
  panY = rect.height / 2 - (minY + contentH / 2) * zoom;
  applyTransform();
}
...
async function openCanvas(path: string): Promise<void> {
  try {
    ...
    canvasPath = path; ensureCanvasTab(path); showCanvas(); renderCanvas(); requestAnimationFrame(() => fitCanvas());
    statusLeft.textContent = path;
    propertiesPanelForCanvas();
  } catch (error) {
    statusLeft.textContent = `Canvas error: ${error instanceof Error ? error.message : String(error)}`;
  }
}

function propertiesPanelForCanvas(): void {
...
fileTree.addEventListener('click', async (event) => {
  ...
  if (name.toLowerCase().endsWith('.canvas')) {
    ...
    const path = await resolveCanvasPathByName(name); if (path) await openCanvas(path);
  } else if (name.toLowerCase().endsWith('.md')) hideCanvas();
}, true);

tabBar.addEventListener('click', (event) => {
  const tab = (event.target as HTMLElement).closest('.tab');
  if (tab && !tab.classList.contains('canvas-tab')) hideCanvas();
}, true);
...
viewport.addEventListener('pointerdown', (event) => {
  ...
  viewport.setPointerCapture(event.pointerId); pan = { x: event.clientX, y: event.clientY, panX, panY }; viewport.classList.add('panning');
});
...
host.querySelector('[data-canvas-action="add-text"]')?.addEventListener('click', () => {
  if (!canvasPath) return;
  const rect = viewport.getBoundingClientRect(); const center = transformPoint(rect.width / 2, rect.height / 2);
  documentState.nodes.push({ id: crypto.randomUUID().replace(/-/g, '').slice(0, 16), type: 'text', text: '', x: center.x - 140, y: center.y - 90, width: 280, height: 180 });
