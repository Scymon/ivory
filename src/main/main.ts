...
async function scanDirectory(root: string, current = root): Promise<VaultEntry[]> {
  const dirents = await fs.readdir(current, { withFileTypes: true }); const entries: VaultEntry[] = [];
  for (const dirent of dirents) {
    if (dirent.name === '.git' || dirent.name === 'node_modules') continue;
    const absolute = path.join(current, dirent.name); const relative = portablePath(path.relative(root, absolute));
...
function resolveVaultPath(relativePath: string): string {
  ...
  const root = path.resolve(vaultRoot); const target = path.resolve(root, relativePath); const relative = path.relative(root, target);
  if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error('Path escapes the open vault.');
  return target;
}

async function snapshot(): Promise<VaultSnapshot | null> { return vaultRoot ? { root:vaultRoot, name:path.basename(vaultRoot), entries:await scanDirectory(vaultRoot) } : null; }

async function startWatcher(): Promise<void> {
  await vaultWatcher?.close(); vaultWatcher = null; if (!vaultRoot) return;
  vaultWatcher = chokidar.watch(vaultRoot,{ignoreInitial:true,ignored:[/(^|[/\\])\.git([/\\]|$)/,/(^|[/\\])node_modules([/\\]|$)/],awaitWriteFinish:{stabilityThreshold:180,pollInterval:50}});
  const emit=(type:VaultChange['type'],absolutePath:string)=>{if(!vaultRoot||!mainWindow||mainWindow.isDestroyed())return;const relative=portablePath(path.relative(vaultRoot,absolutePath));if(!relative||relative.startsWith('..'))return;mainWindow.webContents.send('ivory:vault:change',{type,path:relative} satisfies VaultChange);};
  vaultWatcher.on('add',f=>emit('add',f)).on('change',f=>emit('change',f)).on('unlink',f=>emit('unlink',f)).on('addDir',f=>emit('addDir',f)).on('unlinkDir',f=>emit('unlinkDir',f));
}
...
ipcMain.handle('ivory:file:create-markdown',async(_event,relativePath:string)=>{const normalized=relativePath.toLowerCase().endsWith('.md')?relativePath:`${relativePath}.md`;const target=resolveVaultPath(normalized);await fs.mkdir(path.dirname(target),{recursive:true});await fs.writeFile(target,'',{encoding:'utf8',flag:'wx'});});
